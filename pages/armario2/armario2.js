...
function moveParaArmario1(serial){
  ...
  sim.addEventListener('click', function(){
    url = `http://localhost:11032/api/Armario2/MoverParaArmario1/${serial}` + "/" + operadoraSelected
    fazPost(url)
  });
...
function moveParaDefeito(serial){
  ...
  sim.addEventListener('click', function(){
      let url = "http://localhost:11032/api/Armario2/MoverParaDefeito/";
...
async function main() {
  try {
    const armario3 = await fetchData('http://localhost:11032/api/Armario2');
    $('#tabela').DataTable({
      data: armario3,
      columns: [
        { data: 'serial' },
...
async function modelos() {
  try {
    const data = await fetchData('http://localhost:11032/api/Armario2/Modelos');
